...
class Connector extends EventEmitter {
    ...
    GET_HASH = () => {
        ...
        return crypto.createHash("sha256").update(config.client.password + remoteIPAddress + config.ls.dbPort).digest("hex");
    }

    GET_ADDRESS = () => {

        return config.client.walletAddress;
    }
    ...
    CONNECT_SERVER = async (id) => {
        ...
        try {
            ...
            this.#server[id] = mysql.createPool(config).promise();

            connection = await this.#server[id].getConnection();
            connection.on("error", () => {

                clearInterval(this.#pingInterval[id]);
                this.emit("serverUpdate", id, false);
            });

            this.#pingInterval[id] = setInterval(() => { connection.query("SELECT 1;"); }, 45000);
    ...
    VALIDATE_SERVER = async (id) => {
        ...
        const connection = await this.#server[id].getConnection();
        ...
        if (id == "ls") {
            ...
            const accounts = (await connection.query("SHOW COLUMNS FROM accounts;"))[0];
            const gameservers = (await connection.query("SHOW COLUMNS FROM gameservers;"))[0];

            const lsConfig = this.#config.ls.dbTableColumns;

            accounts.forEach((column) => {

                if (column.Field == lsConfig.accounts.accountUsername || column.Field == lsConfig.accounts.accountPassword)
                    validated++;
            });

            gameservers.forEach((column) => {
            ...
            const characters = (await connection.query("SHOW COLUMNS FROM characters;"))[0];
            const items = (await connection.query("SHOW COLUMNS FROM items;"))[0];
    ...
    /*
        VALIDATE_DATABASE = async () => {
            ...
            const accounts = "ALTER TABLE accounts ADD COLUMN IF NOT EXISTS wallet_address VARCHAR(42) NOT NULL DEFAULT 'not linked';";
            const gameservers = "ALTER TABLE gameservers ADD COLUMN IF NOT EXISTS balance INT(10) UNSIGNED NOT NULL DEFAULT '0';";
            const fiskpay_deposits = "CREATE TABLE IF NOT EXISTS fiskpay_deposits (server_id INT(11) NOT NULL, transaction_hash VARCHAR(66) NOT NULL, character_name VARCHAR(35) NOT NULL, wallet_address VARCHAR(42) NOT NULL, amount INT(10) UNSIGNED NOT NULL, PRIMARY KEY(transaction_hash)) ENGINE = InnoDB DEFAULT CHARSET = utf8;";
            const fiskpay_withdrawals = "CREATE TABLE IF NOT EXISTS fiskpay_withdrawals (server_id INT(11) NOT NULL, transaction_hash VARCHAR(66) NOT NULL, character_name VARCHAR(35) NOT NULL, wallet_address VARCHAR(42) NOT NULL, amount INT(10) UNSIGNED NOT NULL, PRIMARY KEY(transaction_hash)) ENGINE = InnoDB DEFAULT CHARSET = utf8;";
            const fiskpay_temporary = "CREATE TABLE IF NOT EXISTS fiskpay_temporary (server_id INT(11) NOT NULL, character_id INT(10) NOT NULL, amount INT(10) UNSIGNED NOT NULL, refund INT(10) UNSIGNED NOT NULL, PRIMARY KEY(server_id, character_id, refund)) ENGINE = InnoDB DEFAULT CHARSET = utf8;";
    
            const accs = this.#loginServer.query(accounts);
            const gss = this.#loginServer.query(gameservers);
            const deps = this.#loginServer.query(fiskpay_deposits);
            const withs = this.#loginServer.query(fiskpay_withdrawals);
            const temp = this.#loginServer.query(fiskpay_temporary);
    
            await Promise.all([accs, gss, deps, withs, temp]);
            return true;
        }
    */
    /*
    GET_IDS = async () => {
        ...
        let result = await this.#loginServer.query("SELECT server_id FROM gameservers");

        return (result[0].map(key => key.server_id));
    }
