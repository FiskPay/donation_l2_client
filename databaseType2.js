...
class dbClass extends EventEmitter {
    ...
    GET_CHARACTER_BALANCE = async (character) => {
        ...
        return await new Promise((resolve, reject) => {
            ...
            this.#gameServer.query("SELECT `charId` FROM `characters` WHERE `char_name` = ?", [character], async (error, result) => {
                ...
                const data = await new Promise((res) => {
                    ...
                    this.#gameServer.query("SELECT `count` FROM `items` WHERE `owner_id` = ? AND item_id = ? AND `loc` = 'inventory'", [result[0].charId, this.#item], (e, r) => {
    ...
    ADD_REFUND_AND_DECREASE_BALANCE = async (serverid, character, amount, address) => { //SETS AND RETURNS THE REFUND TIMESTAMP
        ...
        return await new Promise((resolve, reject) => {
            ...
            this.#gameServer.query("SELECT `account_name`, `charId` FROM `characters` WHERE `char_name` = ?", [character], async (error, result) => {
                ...
                data = await new Promise((res) => {
                    ...
                    if (data == amount) {
                        ...
                        this.#gameServer.query("DELETE FROM `items` WHERE `owner_id` = ? AND item_id = ? AND `loc` = 'inventory'", [result[0].charId, this.#item], (e) => {
                        ...
                        this.#gameServer.query("UPDATE `items` SET `count` = `count` - ? WHERE `owner_id` = ? AND item_id = ? AND `loc` = 'inventory'", [amount, result[0].charId, this.#item], (e) => {
                ...
                data = await new Promise((res) => {
                    ...
                    this.#loginServer.query("INSERT INTO `donation_temporary` (`server_id`, `owner`, `item`, `amount`, `refund`) VALUES (?, ?, ?, ?, ?)", [serverid, result[0].charId, this.#item, amount, refund], (e, r) => {
            ...
            this.#gameServer.query("SELECT `charId` FROM `characters` WHERE `char_name` = ?", [character], async (error, result) => {
                ...
                data = await new Promise(async (res) => {
                    ...
                    if (data === -1) {
                        ...
                        this.#gameServer.query("INSERT INTO `items` (`owner_id`, `object_id`, `item_id`, `count`, `loc`) VALUES (?, ?, ?, ?, 'inventory')", [result[0].charId, objID, this.#item, amount], (e) => {
        ...
        return await new Promise((resolve, reject) => {
            ...
            this.#gameServer.query("SELECT `charId` FROM `characters` WHERE `char_name` = ?", [character], async (error, result) => {
                ...
                data = await new Promise((res) => {
                    ...
                    this.#loginServer.query("DELETE FROM `donation_temporary` WHERE `server_id` = ? AND `owner` = ? AND item = ? AND `amount` = ? AND `refund` = ?", [serverid, result[0].charId, this.#item, amount, refund], (e) => {
